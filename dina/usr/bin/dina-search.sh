...
# This is a helper script to launch a dialog powered by yad (https://github.com/v1cont/yad) and let the user enter keyword(s) to initiate a web search by launching a search engine of their choice and then launch the search webpage in Dina browser. 
...
TITLE="Dina Search"
ICON="dina-browser"
ENGINES=$(yad --width=50 --center --window-icon="$ICON" --borders 15 --name="${0##*/}" --title="$TITLE" --image="find" --form --field="  Choose search engine:CB" Google\!Bing\!Yahoo\!DuckDuckGo\!searX\!Brave\!Wikipedia\!YouTube\!Baidu\!Yandex\!Naver\!Ecosia --button="<b>Choose</b>"!"find"!"choose a search engine for this search":0 --button="Cancel"!"cancel"!"close this dialog":"pkill -f dina-search.sh")
...
case $ENGINES in
   *)
      ...
      if [ $SELECTION == "Naver" ]
      then
      SEARCHENGINE="https://search.naver.com/search.naver?where=nexearch&query="
      fi
      if [ $SELECTION == "Ecosia" ]
      then
